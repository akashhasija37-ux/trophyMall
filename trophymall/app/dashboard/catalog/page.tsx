...
import { Search, X, CheckCircle2, ShoppingBag, Plus, Minus, Trash2, ArrowRight, ChevronLeft, ChevronRight, SlidersHorizontal, ArrowLeft, Layers, Grid, AlertCircle, ChevronDown, FileText } from "lucide-react";
...
export default function CatalogPage() {
  ...
  // Cart & Order Drawer State & Navigation Tabs
  ...
  const pageSize = 12; // Increased to 12 items per page for better grid layout
  ...
  // Helper to safely get image from either featured_image or image
  const getProductImage = (item: any) => {
    return item?.featured_image || item?.image || null;
  };

  const categoryList = Array.from(new Set(products.map((p) => (p.category || "General").trim()).filter(Boolean))).map((cat) => {
    const catProducts = products.filter((p) => (p.category || "General").trim() === cat);
    return {
      name: cat,
      count: catProducts.length,
      image: getProductImage(catProducts[0]),
    };
  });

  const subCategoryList = selectedCategory 
    ? Array.from(new Set(products.filter((p) => (p.category || "General").trim() === selectedCategory).map((p) => (p.sub_category || p.subcategory || p.subCategory || "Standard").trim()).filter(Boolean))).map((sub) => {
        const subProducts = products.filter((p) => (p.category || "General").trim() === selectedCategory && ((p.sub_category || p.subcategory || p.subCategory || "Standard").trim() === sub));
        return {
          name: sub,
          count: subProducts.length,
          image: getProductImage(subProducts[0]),
        };
      })
    : [];

  const availableBrands = Array.from(new Set(products.map((p) => p.supplier || p.brand || "Default Supplier").filter(Boolean)));
  const discountOptions = [10, 20, 25, 40, 50];
  ...
  // ✅ FILTERS: Category, TM Code / Title Search, Price, Brand, Discount
  const filteredProducts = products.filter((p) => {
    ...
    const tmCode = (p.tm_code || p.sku || "").toLowerCase();
    const matchesSearch = !query || productName.includes(query) || tmCode.includes(query);
    
    const itemBrand = p.supplier || p.brand || "Default Supplier";
    const itemDisc = Number(p.discount || 0);

    const matchesCategoryCard = !selectedCategory || (p.category || "General").trim() === selectedCategory;
    const matchesSubCategoryCard = !selectedSubCategory || (p.sub_category || p.subcategory || "Standard").trim() === selectedSubCategory;
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    if (sortBy === "Better Discount") return Number(b.discount || 0) - Number(a.discount || 0);
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0A] text-zinc-100 font-sans overflow-hidden selection:bg-green-600 selection:text-white">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        {/* STOREFRONT TOP BAR WITH NAVIGATION TABS */}
        <div className="bg-[#121215] border-b border-zinc-800 px-8 py-3 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-6">
            <span className="text-xs font-black tracking-widest text-green-500 uppercase">TROPHY MALL STOREFRONT</span>
            <div className="flex gap-4 text-xs font-semibold">
              <button
                onClick={() => { setActiveTab("catalog"); setViewMode("categories"); setSelectedCategory(null); }}
                className={`pb-1 border-b-2 transition ${activeTab === "catalog" ? "border-green-500 text-white" : "border-transparent text-zinc-400 hover:text-white"}`}
              >
                Product Catalog
              </button>
              <button
                onClick={() => setActiveTab("orders")}
                className={`pb-1 border-b-2 transition ${activeTab === "orders" ? "border-green-500 text-white" : "border-transparent text-zinc-400 hover:text-white"}`}
              >
                Website Orders ({orders.length})
              </button>
            </div>
          </div>

          {/* GLOBAL TM CODE SEARCH */}
          <div className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 px-4 py-2 rounded-xl w-[320px]">
            <Search size={15} className="text-zinc-400" />
            <input
              type="text"
              placeholder="Search by TM Code (e.g. TM-101) or Title..."
              value={search}
              onChange={(e) => {
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex">
          ...
          {activeTab === "catalog" ? (
            <>
              ...
              <aside className="w-72 border-r border-zinc-800 bg-[#0A0A0A] p-6 flex flex-col gap-6 shrink-0 hidden lg:flex overflow-y-auto custom-scrollbar">
                ...
                <div className="flex flex-col gap-3 pb-5 border-b border-zinc-800">
                  <span className="text-[11px] font-black tracking-widest text-white uppercase">SUPPLIER</span>
                  <div className="flex flex-col gap-2.5 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                    {availableBrands.map((brand: string) => {
                      ...
                      const count = products.filter(p => (p.supplier || p.brand || "Default Supplier") === brand).length;
                      return (
                ...
                {/* PRICE RANGE */}
                <div className="flex flex-col gap-3 pb-5 border-b border-zinc-800">
                  <span className="text-[11px] font-black tracking-widest text-white uppercase">MAX PRICE (₹)</span>
                  <input
                    type="number"
                    value={priceRange.max}
                    onChange={(e) => setPriceRange({ ...priceRange, max: Number(e.target.value) || 50000 })}
                    className="w-full bg-[#141416] border border-zinc-700 rounded-xl px-3 py-2 text-xs text-white outline-none"
                  />
                </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-500 mt-1">
                          {viewMode === "categories" && `Select a category to explore items`}
                        ...
                        <div className="relative">
                          ...
                          {isSortOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-[#16161a] border border-zinc-800 rounded-2xl shadow-2xl z-50 py-2">
                              {["Recommended", "What's New", "Better Discount", "Price: High to Low", "Price: Low to High"].map((opt) => (
                                <button
                                  key={opt}
                    ...
                    {categoryList.map((cat, idx) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          const hasSub = products.some(p => (p.category || "").trim() === cat.name && (p.sub_category || p.subcategory || p.subCategory));
                          if (hasSub) {
                            setViewMode("subcategories");
                          } else {
                            setSelectedSubCategory(null);
                      ...
                      >
                        {cat.image && (
                          <div className="absolute inset-0 opacity-20 group-hover:opacity-30 transition-opacity overflow-hidden">
                            <img src={`/uploads/${cat.image}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                        ...
                        <div className="relative z-10 mt-auto">
                          ...
                          <p className="text-xs text-zinc-400 mt-1 flex items-center gap-1">
                            Explore Products <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform" />
                          </p>
                        </div>
                      </div>
                    ))}
                  ...
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in duration-300">
                      {paginated.length > 0 ? (
                        paginated.map((item, idx) => {
                          ...
                          return (
                            <div
                              ...
                              className="group relative bg-[#121215] border border-zinc-800/80 rounded-2xl p-4 cursor-pointer flex flex-col justify-between transition-all duration-300 hover:border-zinc-700 hover:shadow-2xl overflow-hidden"
                            >
                              <div className="flex items-center justify-between z-10 mb-2">
                                <span className="bg-green-950/80 border border-green-800/60 text-green-400 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider">
                                  {conditionBadge}
                                </span>
                                {getStockBadge(stockQty)}
                              </div>

                              <div className="relative w-full h-48 my-2 flex items-center justify-center overflow-hidden rounded-xl bg-zinc-950/40">
                                {productImage ? (
                                  <img
                                  ...
                                  <span className="text-xs text-zinc-600 font-semibold">No Image Available</span>
                                )}
                              </div>

                              <div className="flex items-end justify-between mt-auto pt-4 border-t border-zinc-800/50">
                                <div>
                                  <span className="text-[10px] font-mono text-blue-400 font-bold block mb-0.5">{item.tm_code || item.sku || `TM-${item.id}`}</span>
                                  <h3 className="text-white text-xs font-medium group-hover:text-green-400 transition-colors line-clamp-1">
                                ...
                                <button
                                  ...
                                  className="bg-zinc-800 hover:bg-green-700 text-white p-2 rounded-xl transition-colors shadow-md flex items-center gap-1 text-xs font-medium"
                                >
                                  <Plus size={14} /> Add
                                </button>
                              </div>
                        ...
                        <div className="col-span-4 py-24 text-center text-zinc-500 bg-[#121215] border border-zinc-800 rounded-2xl">
                          <p className="text-sm">No products matching your precise filter selection or TM code search.</p>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 pb-12 border-t border-zinc-800/60 mt-4">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            disabled={page === 1}
                            className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-[#121215] border border-zinc-800 text-zinc-300 hover:bg-zinc-800 hover:text-white disabled:opacity-40 transition-all flex items-center gap-1"
                          >
            ...
            /* ================= WEBSITE ORDERS TAB VIEW ================= */
            <main className="flex-1 p-8 max-w-6xl mx-auto w-full overflow-y-auto">
              <h2 className="text-2xl font-bold text-white mb-2">Website Customer Orders</h2>
              <p className="text-xs text-zinc-400 mb-6">Review orders placed by customers directly through your store catalog.</p>

              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="text-zinc-500 border-b border-zinc-800">
                    <tr>
                      <th className="pb-3">Order ID / Invoice</th>
                      ...
                      <th className="pb-3 text-right">Salesperson</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-800">
                    {orders.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="py-12 text-center text-zinc-500">No website orders found yet.</td>
                      </tr>
                    ) : (
                      orders.map((order, idx) => (
                        <tr key={idx} className="hover:bg-zinc-800/40 transition">
                          <td className="py-4 font-mono font-bold text-blue-400">{order.invoice_id}</td>
                          <td className="text-white font-semibold">{order.customer_name}</td>
                          <td className="text-zinc-300">{order.phone || "-"}</td>
                          <td className="text-right font-black text-white">₹{order.total_amount}</td>
                          ...
                          <td className="text-center text-zinc-400">{order.due_date ? new Date(order.due_date).toLocaleDateString() : "-"}</td>
                          <td className="text-right text-zinc-300">{order.salesperson_name || "Online Store"}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {orderSuccess ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-3">
                  ...
                  <p className="text-zinc-400 text-xs">Thank you for your order. It has been recorded in website orders.</p>
                </div>
              ) : isCheckingOut ? (
                ...
                cart.map((item) => {
                  ...
                  return (
                    <div key={item.product.id} className="bg-[#18181c] border border-zinc-800/80 rounded-2xl p-3.5 flex items-center justify-between gap-3">
                      <div className="w-14 h-14 bg-zinc-950 rounded-xl overflow-hidden shrink-0 border border-zinc-800 flex items-center justify-center">
                        <img src={`/uploads/${itemImg}`} className="max-h-full max-w-full object-contain" onError={(e) => { e.currentTarget.src = "/no-image.png"; }} />
                      </div>
      ...
      {selectedProduct && (() => {
        ...
        const mainImg = getProductImage(selectedProduct);
        const allImages = [mainImg, ...gallery].filter(Boolean);
        const currentImage = allImages[activeImageIndex] || mainImg;
                ...
                <div className="flex flex-col justify-between space-y-6">
                  <div className="space-y-4">
                    <div className="bg-[#1c1c21] border border-zinc-800/80 rounded-2xl p-5 space-y-3.5">
                      ...
                      <div className="flex justify-between text-xs text-zinc-400 border-b border-zinc-800/60 pb-2.5">
                        <span>TM Code / SKU</span>
                        <span className="text-green-400 font-mono font-bold">{selectedProduct.tm_code || selectedProduct.sku || "N/A"}</span>
                      </div>
                      <div className="flex justify-between text-xs text-zinc-400 border-b border-zinc-800/60 pb-2.5">
                      ...
                      <div className="flex justify-between text-xs text-zinc-400">
                        ...
                        <span className="text-white font-medium">{selectedProduct.supplier || "Default Supplier"}</span>
                      </div>
                    </div>
