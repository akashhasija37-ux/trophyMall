...
import { Plus, Search, Download, Eye, Edit, Calendar } from "lucide-react";
...
export default function PurchaseVoucherPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "Pending":
        return (
          <span className="inline-flex items-center justify-center border border-zinc-700/80 text-zinc-400 bg-zinc-800/30 px-2.5 py-0.5 rounded text-xs font-medium min-w-[70px]">
            Pending
          </span>
        );
      default:
        ...
        <div className="flex-1 overflow-y-auto bg-[#0a0a0a] custom-scrollbar">
          ...
          <div className="p-6 max-w-[1400px] mx-auto flex flex-col space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white leading-none mb-2">Purchase Voucher</h2>
                <p className="text-zinc-400 text-sm">3 records</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                ...
                <Link href="/dashboard/new-voucher">
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {mockVouchers.map((voucher) => (
                      <tr key={voucher.id} className="hover:bg-zinc-800/20 transition-colors">
                        <td className="py-4 px-6 text-white font-medium">{voucher.refNo}</td>
                        <td className="py-4 px-6 text-zinc-300">{voucher.party}</td>
                        <td className="py-4 px-6 text-zinc-400">{voucher.date}</td>
                        <td className="py-4 px-6 text-white font-medium">{voucher.amount}</td>
