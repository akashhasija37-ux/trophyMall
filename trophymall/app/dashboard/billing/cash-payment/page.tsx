...
export default function CashPaymentPage() {
  const [records, setRecords] = useState<CashPaymentRecord[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [dateFilter, setDateFilter] = useState("");
  const [isCreating, setIsCreating] = useState(false);

  const [voucherNo, setVoucherNo] = useState(`CPY-2026-${Math.floor(1000 + Math.random() * 9000)}`);
  ...
  const fetchCashPayments = async () => {
    try {
      const res = await fetch("/api/receipts?method=Cash");
      ...
      console.error("Failed to fetch cash payments", err);
    }
  };
  ...
  const handleSaveCashPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAmount("");
      setNarration("");
      setVoucherNo(`CPY-2026-${Math.floor(1000 + Math.random() * 9000)}`);
    } catch (err: any) {
          ...
          <div className="p-6 max-w-[1400px] mx-auto flex flex-col space-y-6">
            ...
            {isCreating ? (
              <div className="flex flex-col gap-4">
                ...
                <form onSubmit={handleSaveCashPayment} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-[#121212] border border-zinc-800/80 rounded-xl p-6 shadow-xl flex flex-col gap-5">
                    ...
                    <div className="mt-4 flex justify-end gap-3 border-t border-zinc-800 pt-4">
                      ...
                      <button type="submit" disabled={loading} className="bg-green-700 hover:bg-green-600 text-white text-sm px-5 py-2.5 rounded-lg flex items-center gap-2">
                        <Save size={16} /> {loading ? "Saving..." : "Save Cash Payment"}
                      </button>
                    </div>
                  </div>
                </form>
              ...
              <div className="bg-[#121212] border border-zinc-800/80 rounded-xl p-6 shadow-xl flex flex-col gap-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                  <div>
                    <h2 className="text-lg font-bold text-white leading-none mb-2">Cash Payment Records</h2>
                      ...
                      {filteredRecords.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan={5} className="py-12 text-center text-zinc-500">
                            No cash payment records found from database.
                          </td>
                        </tr>
                      )}
                    </tbody>
