...
const initialProductMasters: ProductMasterRecord[] = [
  {
    id: "1",
    sku: "TRP-GLD-001",
  ...
  },
  {
    ...
    stock: 8,
    sellingPrice: 4200,
    supplier: "Laser Craft Solutions",
    status: "Active",
  },
  {
    id: "5",
    ...
    stock: 0,
    sellingPrice: 150,
    supplier: "Globe Packaging",
    status: "Inactive",
  },
];

export default function ProductMasterPage() {
  const [records, setRecords] = useState<ProductMasterRecord[]>(initialProductMasters);
  ...
  const [sku, setSku] = useState("TRP-GLD-006");
  ...
  const [rackLocation, setRackLocation] = useState("A-12");
  const [stockStatus, setStockStatus] = useState("");
  const [height, setHeight] = useState("30cm");
  const [width, setWidth] = useState("12cm");
  const [weight, setWeight] = useState("450g");
  const [productSize, setProductSize] = useState("");
  const [basePrintSize, setBasePrintSize] = useState("5x3 inch");
  ...
  const handleSaveNewProduct = (e: React.FormEvent) => {
    ...
    // Reset Form
    setProductName("");
    setSku(`TRP-GLD-00${records.length + 2}`);
    setBarcode("");
    setCategory("");
    setSubCategory("");
    setCustomCategory("");
        ...
        <div className="flex-1 overflow-y-auto bg-[#0a0a0a] custom-scrollbar relative">
          ...
          <div className="px-6 py-4 flex justify-between items-start border-b border-zinc-800/60">
            <div>
              <p className="text-xs text-zinc-500 mb-1">
                Masters / <span className="text-zinc-300">Product Master</span>
              </p>
              <h1 className="text-xl font-bold text-white">Product Master</h1>
            </div>
          ...
          <div className="p-6 max-w-[1600px] mx-auto flex flex-col space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white leading-none mb-2">Product Master</h2>
                <p className="text-zinc-400 text-sm">Complete product catalog — add, edit, and manage all products with stock details</p>
              </div>
                    ...
                    {filteredRecords.length > 0 ? (
                      filteredRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-[#1a1a1c] transition-colors">
                          <td className="py-4 px-6 text-zinc-400 font-mono text-xs">{record.sku}</td>
                          <td className="py-4 px-6 font-semibold text-white">{record.productName}</td>
                          <td className="py-4 px-6 text-zinc-300">{record.category}</td>
                          <td className="py-4 px-6 text-zinc-300 font-medium">{record.stock} units</td>
                          <td className="py-4 px-6 font-semibold text-white">{formatCurrency(record.sellingPrice)}</td>
                          <td className="py-4 px-6 text-zinc-400">{record.supplier}</td>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#161618] border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
            ...
            <div className="px-6 py-4 flex items-center justify-between border-b border-zinc-800/85 shrink-0">
              <h3 className="text-base font-bold text-white">Add New — Product Master</h3>
              <button 
            ...
            {/* Modal Form Body */}
            <form onSubmit={handleSaveNewProduct} className="p-6 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
              
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Product Name *</label>
                <input
                  type="text"
                  placeholder="e.g. Gold Trophy 12 Inch"
                  value={productName}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Sub Category</label>
                <input
                  type="text"
                  placeholder="Gold Trophies"
                  value={subCategory}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Custom Category</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Purchase Price (₹)</label>
                <input
                  type="number"
                  placeholder="0"
                  value={purchasePrice}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                >
                  <option value="" disabled>Select...</option>
                  <option value="Raj Metals Pvt Ltd">Raj Metals Pvt Ltd</option>
                  <option value="Delhi Trophy House">Delhi Trophy House</option>
                  <option value="Acrylic World">Acrylic World</option>
                </select>
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Warehouse</label>
                <select
                  value={warehouse}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Rack Location</label>
                <input
                  type="text"
                  value={rackLocation}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Stock Status</label>
                <select
                  value={stockStatus}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Product Size</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Base Print Size</label>
                <input
                  type="text"
                  value={basePrintSize}
                  onChange={(e) => setBasePrintSize(e.target.value)}
                  className="w-full bg-[#1e1e22] border border-zinc-700 focus:border-green-600 rounded-lg px-3.5 py-2.5 text-sm text-zinc-200 outline-none transition-colors"
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Badge</label>
                <select
                  value={badge}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-[#1e1e22] border border-zinc-700 focus:border-green-600 rounded-lg p-3.5 text-sm text-zinc-200 outline-none transition-colors resize-none"
                />
              </div>
