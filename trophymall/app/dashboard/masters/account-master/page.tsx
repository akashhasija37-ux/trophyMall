...
const initialAccountMasters: AccountMasterRecord[] = [
  {
    id: "1",
    code: "ACC-001",
...
export default function AccountMasterPage() {
  const [records, setRecords] = useState<AccountMasterRecord[]>(initialAccountMasters);
  ...
  const [accountCode, setAccountCode] = useState("ACC-006");
  ...
  const handleSaveNewAccount = (e: React.FormEvent) => {
    e.preventDefault();
    if (!accountName || !accountGroup) return;

    const newRecord: AccountMasterRecord = {
      id: Date.now().toString(),
      code: accountCode,
      accountName,
      group: accountGroup,
      openingBalance: Number(openingBalance) || 0,
      type: balanceType || "General",
      status: "Active",
    };
    ...
    // Reset Form
    setAccountName("");
    setAccountCode(`ACC-00${records.length + 2}`);
    setAccountGroup("");
    setOpeningBalance("");
    setBalanceType("");
  };
        ...
        <div className="flex-1 overflow-y-auto bg-[#0a0a0a] custom-scrollbar relative">
          ...
          <div className="px-6 py-4 flex justify-between items-start border-b border-zinc-800/60">
            <div>
              <p className="text-xs text-zinc-500 mb-1">
                Masters / <span className="text-zinc-300">Account Master</span>
              </p>
              <h1 className="text-xl font-bold text-white">Account Master</h1>
            </div>
          ...
          <div className="p-6 max-w-[1600px] mx-auto flex flex-col space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white leading-none mb-2">Account Master</h2>
                <p className="text-zinc-400 text-sm">Manage individual ledger accounts</p>
              </div>
            ...
            <div className="bg-[#121212] border border-zinc-800/80 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xl">
              ...
              <div className="relative w-full md:w-72">
                <input
                  type="text"
                  placeholder="Search Account Master..."
                  value={searchTerm}
                ...
                <table className="w-full text-left border-collapse whitespace-nowrap">
                  <thead>
                    <tr className="bg-[#18181a] text-zinc-400 text-xs uppercase tracking-wider border-b border-zinc-800">
                      ...
                      <th className="py-3.5 px-6 font-semibold">Account Name</th>
                      <th className="py-3.5 px-6 font-semibold">Group</th>
                      <th className="py-3.5 px-6 font-semibold">Opening Balance</th>
                    ...
                    {filteredRecords.length > 0 ? (
                      filteredRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-[#1a1a1c] transition-colors">
                          ...
                          <td className="py-4 px-6 font-semibold text-white">{record.accountName}</td>
                          <td className="py-4 px-6 text-zinc-300">{record.group}</td>
                          <td className="py-4 px-6 font-semibold text-white">{formatCurrency(record.openingBalance)}</td>
                      ...
                      <tr>
                        <td colSpan={7} className="py-12 text-center text-zinc-500">
                          No account master records found matching your filters.
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#161618] border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
            ...
            <div className="px-6 py-4 flex items-center justify-between border-b border-zinc-800/80">
              <h3 className="text-base font-bold text-white">Add New — Account Master</h3>
              <button 
            ...
            {/* Modal Form Body */}
            <form onSubmit={handleSaveNewAccount} className="p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Account Group *</label>
                <select
                  value={accountGroup}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Opening Balance (₹)</label>
                <input
                  type="number"
                  placeholder="0.00"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Balance Type</label>
                <select
                  value={balanceType}
