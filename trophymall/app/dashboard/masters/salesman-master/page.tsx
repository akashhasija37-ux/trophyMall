...
const initialSalesmanMasters: SalesmanMasterRecord[] = [
  {
    id: "1",
    code: "SM-001",
    name: "Rajesh Kumar",
...
export default function SalesmanMasterPage() {
  const [records, setRecords] = useState<SalesmanMasterRecord[]>(initialSalesmanMasters);
  ...
  const handleSaveNewSalesman = (e: React.FormEvent) => {
    e.preventDefault();
    if (!employeeName || !mobileNumber) return;

    const newRecord: SalesmanMasterRecord = {
      id: Date.now().toString(),
      code: `SM-00${records.length + 1}`,
      name: employeeName,
      mobile: mobileNumber,
      branch: branch || "Mumbai HQ",
      commPct: Number(commissionPct) || 2.5,
      target: Number(monthlyTarget) || 500000,
      status: "Active",
    };
        ...
        <div className="flex-1 overflow-y-auto bg-[#0a0a0a] custom-scrollbar relative">
          ...
          <div className="px-6 py-4 flex justify-between items-start border-b border-zinc-800/60">
            <div>
              <p className="text-xs text-zinc-500 mb-1">
                Masters / <span className="text-zinc-300">Salesman Master</span>
              </p>
              <h1 className="text-xl font-bold text-white">Salesman Master</h1>
            </div>
          ...
          <div className="p-6 max-w-[1600px] mx-auto flex flex-col space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white leading-none mb-2">Salesman Master</h2>
                <p className="text-zinc-400 text-sm">Manage salespeople and their performance targets</p>
              </div>
            ...
            <div className="bg-[#121212] border border-zinc-800/80 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xl">
              ...
              <div className="relative w-full md:w-72">
                <input
                  type="text"
                  placeholder="Search Salesman Master..."
                  value={searchTerm}
                ...
                <table className="w-full text-left border-collapse whitespace-nowrap">
                  <thead>
                    <tr className="bg-[#18181a] text-zinc-400 text-xs uppercase tracking-wider border-b border-zinc-800">
                      ...
                      <th className="py-3.5 px-6 font-semibold">Comm%</th>
                    ...
                    {filteredRecords.length > 0 ? (
                      filteredRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-[#1a1a1c] transition-colors">
                          ...
                          <td className="py-4 px-6 font-semibold text-white">{record.name}</td>
                          <td className="py-4 px-6 text-zinc-300">{record.mobile}</td>
                          <td className="py-4 px-6 text-zinc-400">{record.branch}</td>
                          <td className="py-4 px-6 text-zinc-200 font-medium">{record.commPct}%</td>
                          <td className="py-4 px-6 font-semibold text-white">{formatCurrency(record.target)}</td>
                      ...
                      <tr>
                        <td colSpan={8} className="py-12 text-center text-zinc-500">
                          No salesman master records found matching your filters.
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#161618] border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
            ...
            <div className="px-6 py-4 flex items-center justify-between border-b border-zinc-800/85 shrink-0">
              <h3 className="text-base font-bold text-white">Add New — Salesman Master</h3>
              <button 
            ...
            {/* Modal Form Body */}
            <form onSubmit={handleSaveNewSalesman} className="p-6 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
              
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Employee Name *</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Commission %</label>
                <input
                  type="number"
                  step="0.1"
                  placeholder="2.5"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Monthly Target (₹)</label>
                <input
                  type="number"
                  placeholder="500000"
                  value={monthlyTarget}
