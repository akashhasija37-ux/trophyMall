...
const initialAccountGroups: AccountGroupRecord[] = [
  {
    id: "1",
    groupName: "Sales Income",
    type: "Income",
    parentGroup: "None",
    ledgers: 4,
    status: "Active",
...
export default function AccountGroupPage() {
  const [records, setRecords] = useState<AccountGroupRecord[]>(initialAccountGroups);
  ...
  const handleSaveNewGroup = (e: React.FormEvent) => {
    ...
    const newRecord: AccountGroupRecord = {
      id: Date.now().toString(),
      groupName,
      type: groupType,
      parentGroup: parentGroup || "None",
      ledgers: 0,
      status: "Active",
      description,
    };
  ...
  const filteredRecords = records.filter((item) => {
    const matchesSearch =
      ...
      item.parentGroup.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="flex-1 overflow-y-auto bg-[#0a0a0a] custom-scrollbar relative">
          ...
          <div className="px-6 py-4 flex justify-between items-start border-b border-zinc-800/60">
            <div>
              <p className="text-xs text-zinc-500 mb-1">
                Masters / <span className="text-zinc-300">Account Group</span>
              </p>
              <h1 className="text-xl font-bold text-white">Account Group</h1>
            </div>
          ...
          <div className="p-6 max-w-[1600px] mx-auto flex flex-col space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white leading-none mb-2">Account Group</h2>
                <p className="text-zinc-400 text-sm">Manage accounting groups for chart of accounts</p>
              </div>
                ...
                <table className="w-full text-left border-collapse whitespace-nowrap">
                  <thead>
                    <tr className="bg-[#18181a] text-zinc-400 text-xs uppercase tracking-wider border-b border-zinc-800">
                      <th className="py-3.5 px-6 font-semibold">Group Name</th>
                      <th className="py-3.5 px-6 font-semibold">Type</th>
                      <th className="py-3.5 px-6 font-semibold">Parent Group</th>
                      <th className="py-3.5 px-6 font-semibold">Ledgers</th>
                    ...
                    {filteredRecords.length > 0 ? (
                      filteredRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-[#1a1a1c] transition-colors">
                          <td className="py-4 px-6 font-semibold text-white">{record.groupName}</td>
                          <td className="py-4 px-6 text-zinc-300">{record.type}</td>
                          <td className="py-4 px-6 text-zinc-400">{record.parentGroup}</td>
                          <td className="py-4 px-6 text-zinc-200">{record.ledgers}</td>
                      ...
                      <tr>
                        <td colSpan={6} className="py-12 text-center text-zinc-500">
                          No account groups found matching your filters.
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#161618] border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
            ...
            <div className="px-6 py-4 flex items-center justify-between border-b border-zinc-800/80">
              <h3 className="text-base font-bold text-white">Add New — Account Group</h3>
              <button 
            ...
            {/* Modal Form Body */}
            <form onSubmit={handleSaveNewGroup} className="p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Group Type *</label>
                <select
                  value={groupType}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">Parent Group</label>
                <select
                  value={parentGroup}
